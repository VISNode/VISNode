...
/**
 * The challenge submission list panel
 */
public class ChallengeSubmissionListPanel extends JPanel {
     ...
     */
    private ChallengeSubmissionListPanel(Mission mission) {
        super();
    ...
    public static void showDialog(Mission mission) {
        Messages.get().message("challenge.solutions").subscribe((msg) -> {
            WindowFactory.modal().title(msg).create((container) -> {
                ...
                container.add(new ChallengeSubmissionListPanel(mission));
            }).setVisible(true);
        }).dispose();
    }
    ...
    private JComponent buildList() {
        ...
        list.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                SwingUtilities.getWindowAncestor(ChallengeSubmissionListPanel.this).dispose();
        ...
        try {
            MissionUserRepository.get().getAll(mission.getId()).stream().
                    sorted((a, b) -> b.getUser().getName().compareTo(a.getUser().getName())).
                    forEach((obj) -> {
                        model.addElement(obj);
                    });
        } catch (RepositoryException ex) {
            Logger.getLogger(ChallengeSubmissionListPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
        list.setModel(model);
    ...
    private class CellRenderer implements ListCellRenderer<MissionUser> {
        ...
        public Component getListCellRendererComponent(JList<? extends MissionUser> list, MissionUser value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            info.add(label);
            info.add(new JLabel(String.format("Status: %s", value.getStatusFormat())));
